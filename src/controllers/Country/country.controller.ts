...
export const createCountryController = async (
  ...
  try {
    ...
    if (!name || !code) {
      throw new BadRequestError((res as any)._t("country.name_code_required"));
    }

    const countryInfo = getCountryByName(name.en);
    if (!countryInfo) {
      throw new BadRequestError((res as any)._t("country.invalid_name"));
    }

    if (countryInfo.code.toUpperCase() !== code.toUpperCase()) {
      throw new BadRequestError((res as any)._t("country.code_mismatch"));
    }

    if (!Array.isArray(cities) || cities.length === 0) {
      throw new BadRequestError((res as any)._t("country.cities_required"));
    }

    const cityNames = cities.map((c) => c.name.en.toLowerCase());
    if (cityNames.length !== new Set(cityNames).size) {
      throw new BadRequestError((res as any)._t("country.duplicate_cities"));
    }
    ...
    if (existingCountry) {
      throw new BadRequestError((res as any)._t("country.already_exists"));
    }
    ...
    res.status(201).json({
      message: (res as any)._t("country.created"),
...
export const getAllCountriesController = async (
  ...
  try {
    ...
    const response = {
      message:
        countries.length === 0
          ? (res as any)._t("country.no_countries")
          : (res as any)._t("country.fetched_all"),
      total,
...
export const getOneCountryController = async (
  ...
  try {
    ...
    if (!country) {
      throw new NotFoundError((res as any)._t("country.not_found"));
    }

    res.json({
      message: (res as any)._t("country.fetched"),
...
export const deleteCountryController = async (
  ...
  try {
    ...
    if (!country) throw new NotFoundError((res as any)._t("country.not_found"));
    if (country.is_deleted)
      throw new BadRequestError((res as any)._t("country.already_deleted"));
    ...
    res.json({
      message: (res as any)._t("country.deleted"),
    });
  } catch (error) {
    next(error);
  }
...
export const updateCountryController = async (
  ...
  try {
    ...
    if (!updateData || Object.keys(updateData).length === 0) {
      throw new BadRequestError((res as any)._t("country.update_required"));
    }
    ...
    res.json({
      message: (res as any)._t("country.updated"),
      country: localizeData(country, locale),
    });
  } catch (error) {
    next(error);
