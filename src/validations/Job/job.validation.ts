...
type RequiredMessages = { en: string; ar: string };
...
const listItemSchemaFactory = (t: TranslationFunction): Joi.ObjectSchema =>
  Joi.object({
...
export const createJobSchema = (req: Request): Joi.ObjectSchema => {
...
export const updateJobSchema = (req: Request): Joi.ObjectSchema => {
  const t: TranslationFunction =
    (req as any).__ || (req as any)._t || ((key: string) => key);
  const listItemSchema = listItemSchemaFactory(t);
